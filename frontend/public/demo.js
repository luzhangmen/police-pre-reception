/**
 * Standalone map demo (GitHub Pages). Uses SCUT University Town POI coordinates.
 */
const CAMPUS = window.CAMPUS_COORDS;
const CAMPUS_REGION = CAMPUS.region;
...
const DEMO_CASES = [
  ...
  },
  {
    ...
    poiKey: "xiaobeiStreet",
    text:
      "3月9日凌晨，我和同学在广州大学城小贝大街吃大排档，喝酒后和邻桌发生口角，" +
...
let mapEngine = null;
let mapOverlays = [];
let useGcj02 = Boolean(amapKey);

function caseMapLocation(caseData) {
  return window.campusMapLocation(caseData.poiKey, useGcj02 ? "gcj02" : "wgs84");
}

function formatCoord(loc) {
  const sys = loc.coord_system === "gcj02" ? "GCJ-02（高德）" : "WGS-84（OSM）";
  return `${sys}：${loc.lat.toFixed(6)}°N, ${loc.lng.toFixed(6)}°E`;
}
...
async function initMapEngine() {
  ...
  const center = CAMPUS.center.wgs84;
  mapEngine = {
    type: "leaflet",
    instance: L.map("map", { zoomControl: true }).setView([center.lat, center.lng], 16),
    layer: L.layerGroup(),
  };
...
async function renderLeaflet(locations) {
  ...
  locations.forEach((loc, index) => {
    ...
    marker.bindPopup(`<strong>${loc.display_name}</strong><br/><small>${formatCoord(loc)}</small>`);
    mapOverlays.push(marker);
  ...
  locations.forEach((loc, index) => {
    ...
    li.innerHTML = `
      ...
      <div>
        ...
        <div class="muted">${formatCoord(loc)}</div>
...
async function showCase(caseData) {
  ...
  const locations = [caseMapLocation(caseData)];
